...
export default class TorrentAddition {
    ...
    constructor(service, client, torrentsDb, gallery, emitter) {
    ...
    seed(files) {
        ...
        torrent.on('error', err => {
            ...
            if(!this.gallery.getTileByUri(torrentId).item) {
                torrent.client.remove(torrentId, (param) => {
                    if(files.length > 0) {
                        scope.seed(files);
                    } else {
                        scope.log('Cannot re-seed empty filelist ' + files);
                        scope.seed(fileBak);
                    }
                });
            }
    ...
    addToDom(torrent) {
        ...
        torrent.files.forEach(file => {
            scope.gallery.addMediaToDom(file, torrent);
        });
    ...
    warning(t, err) {
        ...
        const isMemStoreEmpty = setInterval(()=>{
            ...
            if(empty){
                ...
                //this.log(`[${torrent.infoHash}] Destroying torrent`);
