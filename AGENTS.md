...
rm /tmp/gcp-key.json
```
GCP project: `photogroup-215600`, zone: `asia-east2-a`, VM instance: `main`. Service account: `github-photogroup-app@photogroup-215600.iam.gserviceaccount.com`. Deployment scripts: `deploy-docker.sh`, `deploy-nginx.sh`, `deploy-app.sh`. See `DEPLOYMENT.md` for full details.

The service account is scoped for deployment only (SSH/SCP to the VM). Commands like `gcloud compute instances list` will fail with permission errors — this is expected. Use the deploy scripts directly.
