...
gcloud compute ssh $INSTANCE --zone $ZONE --command "rm -r pg"
gcloud compute ssh $INSTANCE --zone $ZONE --command "mkdir pg"
gcloud compute scp --recurse ./bin/* $INSTANCE:./pg/ --zone $ZONE
gcloud compute ssh $INSTANCE --zone $ZONE --command "sudo lsof -ti:8081 | sudo xargs kill"
